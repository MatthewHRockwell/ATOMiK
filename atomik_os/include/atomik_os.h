...
/* font_aa.c — v0.38-K anti-aliased bitmap atlas (.atomik_font).
 *
 * Host-generated by tools/font_pack.py from a TTF, loaded once at
 * startup, rendered via mask-blit through draw_blend_pixel.  No
 * runtime FreeType.  Pixel font (above) stays for terminal / debug;
 * AA font is used for premium chrome (wordmark, panel headers, big
 * metrics, hero labels).
 *
 * Font IDs are stable across the lifetime of the process: small
 * (14 px UI label), medium (18 px panel chrome), and large (28 px
 * display / hero / big metric). */
typedef enum {
...
int  font_aa_loaded(font_aa_id_t id);    /* 1 if atlas present, else 0       */
int  text_width_aa(font_aa_id_t id, const char *s);
int  text_height_aa(font_aa_id_t id);
int  text_ascender_aa(font_aa_id_t id);
void draw_text_aa(font_aa_id_t id, int x, int y, const char *s, pixel_t color);
