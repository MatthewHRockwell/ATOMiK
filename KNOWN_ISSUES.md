...
**Fix:** Updated testbench to present read addresses at negedge (setup time), register on posedge, and check after. Also corrected reset expectations — BSRAM has no reset, x1-x31 retain values through reset. Result: 38/38 PASS.
...
**Symptom:** AUIPC (Add Upper Immediate to PC) instruction was reported to generate incorrect addresses during early Phase 3 bringup.

**Root Cause:** Likely fixed by one of the CPU bus/control fixes during Phase 3 (V3-009 power-on reset, V3-010 bus arbiter crosstalk, V3-015 LSU bus output glitches, V3-016 CPU hang on MMIO). The exact fix is uncertain since the issue was deferred and resolved as a side effect.

**Verification:**
- `rv64ui-p-auipc` compliance test: PASS (Verilator, 53/54 suite)
- Production firmware uses AUIPC at 6+ call sites (`la gp`, `la sp`, data copy addresses, ctors)
- Boot test confirms correct values: gp=0x40000840 (`__global_pointer$`), sp=0x400010b0 (`_stack_start`)
- All firmware tests pass on hardware (9/9 ATOMiK, 10/10 Phase 2, 6/6 Display)
