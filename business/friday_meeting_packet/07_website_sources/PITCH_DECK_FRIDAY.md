...
| 4 | 1:35-2:10 | Market Opportunity | Large pressure pool, focused wedge, strategic optionality. | `$1T+` TAM context, `$112B-$169B` SAM context, `$10M-$40M` entry wedge; not a ceiling. | TAM/SAM are adjacent spend pools; entry wedge is founder-prepared planning context, not forecast or ceiling. |
...
| 6 | 2:45-3:15 | Competitive Landscape | Alternatives are real; proof-bound IP is the wedge. | More silicon, processor/IP incumbents, software/status quo alternatives, plus Lean4-checked formal algebra + FPGA validation + IP/proof registry. | Names are alternatives, not partnership or displacement claims; formal claims stay tied to audited properties. |
...
| 10 | 4:40-5:10 | Live Proof | Current demo surface running on Zynq hardware. | Show current Zynq UI artifact. | Screenshot is not customer workload proof. |
| 11 | 5:10-5:55 | Proof Stack | Real, specific, evidence-bounded. | Zynq UI, Linux-to-FPGA, AX7020 matrix, formal/synthesis context. | No universal speedup or downstream outcome claim. |
| 12 | 5:55-6:35 | Commercial Path | Show the problem path; measure fit. | Show constrained path, evaluate fit, show measured evidence, or call no-fit. | Design-partner work and licensing diligence only after proof. |
...
| 15 | 8:00-8:30 | Team | Founder-led proof with commercial translation. | Matt owns deck/proof/ask; Allison supports buyer/commercial translation and feedback capture where comfortable. | Allison is not expected to present the technical deck. |
...
- Entry wedge: founder-prepared early annual revenue path if paid evaluations, design partners, and licensing convert. It is not a forecast or ceiling.
