...
Proof story: ATOMiK's current proof stack is not a universal performance claim. It is an artifact-bound stack showing: (1) a current Zynq UI/demo surface captured live from the framebuffer, (2) a Linux userspace-to-FPGA primitive path with algebraic checks passing, (3) a live-measured AX7020 workload matrix with wins, losses, and caveats, (4) a live-measured parallel-bank throughput substrate that scales 1/2/4/8x with allocated banks on silicon, and (5) a version map and claims registry that prevent overclaiming. The next evidence gate is customer-representative Zynq workload validation.
...
| `11-current-live-atomik-desk-v040a-workload.png` | HARDWARE_VALIDATED | Workload surface with Resource Fabric on real data | ATOMiK Desk v0.40-A on Zynq hardware with apps open and the Fabric driven by real measured on-board data. | That it is an interactive session; the in-flight interactive demo and USB input are not board-confirmed. |
| `11-current-live-atomik-desk-v040a-workload.caption.md` | Caption / guardrail | Safe workload-surface caption | Static framebuffer capture, not an interactive session. | Interactive demo, USB input, or customer/production metrics. |
| `PARALLEL_BANKS_HARDWARE_VALIDATED.md` | LIVE_MEASURED | AX7020 parallel-bank throughput | On the AX7020, an 8-bank ATOMiK accumulator scaled 1/2/4/8x with allocated banks, byte-identical to a software recompute. | A customer-workload speedup, power, thermal, or production outcome. |
| `HDMI_1080P60_IMPOSSIBLE.md` | HARDWARE_VALIDATED | Zynq HDMI display ceiling | The ATOMiK Desk UI renders at 1080p30 over HDMI on the AX7020. | 1080p60 on this board (OSERDES2 pulse-width limit makes it physically impossible). |
| `09-current-live-atomik-desk-v039k.png` | HARDWARE_VALIDATED (superseded) | Prior Zynq UI screenshot, retained as history | ATOMiK Desk v0.39-K was a prior Zynq hardware UI artifact (superseded by v0.40-A). | That it is the current proof image. |
| `09-current-live-atomik-desk-v039k.caption.md` | Caption / guardrail (superseded) | Prior screenshot caption | ATOMiK Desk v0.39-K showed live prototype/demo surface and UI direction. | That screenshot metrics are customer or production metrics. |
