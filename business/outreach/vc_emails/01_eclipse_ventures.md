...
**Subject: 1 Gops/s on a $13.50 FPGA — formally verified semiconductor IP**
...
I built a delta-state computation architecture that replaces full-state updates with XOR-based accumulation. Every operation completes in a single clock cycle with zero carry chains. The result: 1 billion operations/second on a $13.50 Tang Nano 9K FPGA.
...
The business model is IP licensing (ARM-style). Applications span HFT, IoT sensor fusion, database replication, and video processing. I'm targeting a $2M seed to fund ASIC tape-out and first commercial licenses.
