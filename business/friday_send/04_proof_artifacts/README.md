...
Proof story: ATOMiK's current proof stack is not a universal performance claim. It is an artifact-bound stack showing: (1) a current Zynq UI/demo surface, (2) a Linux userspace-to-FPGA primitive path with algebraic checks passing, (3) a live-measured AX7020 workload matrix with wins, losses, and caveats, and (4) a version map and claims registry that prevent overclaiming. The next evidence gate is customer-representative Zynq workload validation.
...
| `VERSION_MAP.md` | Governance | Explains v0.39-K, v0.33-D, zynq-linux-v1, and SD boot status | ATOMiK separates UI artifacts, board-run matrices, frozen Linux validation, and build artifacts by version. | That all versions prove the same thing. |
| `09-current-live-atomik-desk-v039k.png` | HARDWARE_VALIDATED | Current Zynq demo surface screenshot | ATOMiK Desk v0.39-K is a current Zynq hardware UI artifact. | Customer workload performance, uptime, production maturity, battery, heat, water, power, or footprint outcomes. |
| `09-current-live-atomik-desk-v039k.caption.md` | Caption / guardrail | Safe screenshot caption | ATOMiK Desk v0.39-K shows live prototype/demo surface and UI direction. | That screenshot metrics are customer or production metrics. |
...
- `PROOF_CARDS.md`
- `VERSION_MAP.md`
- `09-current-live-atomik-desk-v039k.png` with `09-current-live-atomik-desk-v039k.caption.md`
- `evidence-labels.md`
- `AX7020_SUMMARY.md`
