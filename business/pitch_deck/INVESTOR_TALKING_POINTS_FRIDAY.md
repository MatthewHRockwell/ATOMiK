# ATOMiK Friday Talking Points

INTERNAL PRESENTER USE - not an investor/customer leave-behind unless intentionally shared.

Role: primary live script for the 10-minute pitch and Q&A. Expanded prep and Q&A backup: `ATOMiK_Talking_Points_INTERNAL.md`.
...
Hardware-validated Linux userspace-to-FPGA accelerator checks (16/16 PASS, docs/LINUX_USERSPACE_PROOF.md), a HARDWARE_VALIDATED recorded Zynq prototype UI screenshot (v0.39-K, results/claims_registry.yaml), live-measured AX7020 perf matrix artifacts with caveats, 95-second JTAG-assisted boot + 1080p HDMI (hardware/zynq/results/PHASE_9_PROOF.md), and formal proof work in the repo. The currently validated boot path is hardware/JTAG-assisted; standalone SD boot is BUILD_ARTIFACT only and remains active bring-up until promoted by a recorded power-on artifact.

**If asked about standalone SD boot:** "The current validated path is hardware/JTAG-assisted; standalone SD boot is active bring-up." Do not claim it boots or is validated.
