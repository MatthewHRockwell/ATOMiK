// ATOMiK v3 Performance Benchmark Suite
//
// Emits machine-parseable ##PERF lines over UART for automated capture.
// All timing via rdcycle CSR (64-bit). Statistics computed on host (Python).
//
// Categories:
//   A: ATOMiK core ops (custom instruction latency)
//   B: Memory operations (size sweep, 64-bit words)
//   C: Burst & scaling
//   D: CPU baselines
//
// Adapted from v2 MMIO-based benchmark for v3 custom instructions.
// Key difference: v3 uses direct-wire custom instructions (no bus latency).
...
// Size sweep in bytes (max 256 = 32 uint64_t words to fit static buffers)
...
// Checkpoint word counts (64-bit words)
static const uint32_t ckpt_sizes[] = { 4, 8, 16, 32 };
#define N_CKPT_SIZES 4

// ---------------------------------------------------------------------------
// Buffers (256 bytes each — 32 x uint64_t)
// ---------------------------------------------------------------------------

static uint64_t perf_buf_a[32];
static uint64_t perf_buf_b[32];
...
// Fill buffer with deterministic 64-bit pattern
static void fill_pattern(uint64_t *buf, uint32_t n_words)
{
    for (uint32_t i = 0; i < n_words; i++)
        buf[i] = 0xA5A5A5A5A5A5A5A5ULL ^ (uint64_t)i;
}

// ---------------------------------------------------------------------------
// Category A: ATOMiK Core Ops (Custom Instructions)
// ---------------------------------------------------------------------------
...
static void bench_atomik_ops(void)
{
    ...
    for (int iter = 1; iter <= ATOMIK_OPS_ITER; iter++) {
        // atomik_load (custom instruction: ATOMIK.LOAD)
        atomik_init(0);
        c0 = cycles();
        ...
        atomik_init(0);
        c0 = cycles();
        atomik_load_state(0, 0xAAAAAAAA55555555ULL);
        atomik_accumulate(0, 0x5555555FAAAAAAAULL);
        val = atomik_state(0);
        c1 = cycles();
        mini_printf("##PERF:test=atomik_roundtrip,iter=%d,cycles=%u\n", iter, c1 - c0);

        // atomik_swap (v3-specific: ATOMIK.SWAP custom instruction)
        atomik_init(0);
        atomik_load_state(0, 0xCAFEBABEDECAF000ULL);
        atomik_accumulate(0, 0xFF);
        c0 = cycles();
        val = atomik_checkpoint(0);
        c1 = cycles();
        mini_printf("##PERF:test=atomik_swap,iter=%d,cycles=%u\n", iter, c1 - c0);
    }
}

// ---------------------------------------------------------------------------
// Category B: Memory Operations (size sweep, 64-bit)
// ---------------------------------------------------------------------------
...
static void bench_memory_ops(void)
{
    ...
    fill_pattern(perf_buf_a, 32);

    for (int si = 0; si < N_MEM_SIZES; si++) {
        uint32_t sz = mem_sizes[si];
        uint32_t n_words = sz / 8;  // 64-bit words
        ...
        for (int iter = 1; iter <= MEMORY_OPS_ITER; iter++) {
            ...
            // fingerprint (via custom instructions)
            c0 = cycles();
            ...
            // change_detect (no change case — hardware fingerprint comparison)
            uint64_t saved_fp = atomik_fingerprint(0, perf_buf_a, n_words);
            c0 = cycles();
...
static void bench_burst_scaling(void)
{
    ...
    // Burst accumulates (custom instruction throughput)
    for (int bi = 0; bi < N_BURST_COUNTS; bi++) {
        ...
        for (int iter = 1; iter <= BURST_ITER; iter++) {
            ...
            for (uint32_t j = 0; j < count; j++)
                atomik_accumulate(0, (uint64_t)j);
            c1 = cycles();
    ...
    for (int ci = 0; ci < N_CKPT_SIZES; ci++) {
        ...
        for (int iter = 1; iter <= BURST_ITER; iter++) {
            c0 = cycles();
            volatile uint64_t fp = atomik_fingerprint(0, perf_buf_a, n_words);
            c1 = cycles();
            (void)fp;
    ...
    for (int ci = 0; ci < N_CKPT_SIZES; ci++) {
        uint32_t n_words = ckpt_sizes[ci];
        uint64_t expected = atomik_fingerprint(0, perf_buf_a, n_words);
...
static void bench_cpu_baselines(void)
{
    ...
    for (int iter = 1; iter <= CPU_ITER; iter++) {
        ...
        // xor_bench (64-bit xorshift — RV64I native width)
        ...
        c1 = cycles();
        mini_printf("##PERF:test=xor_bench,count=1000,iter=%d,cycles=%u,chksum=0x%lx\n",
                    iter, c1 - c0, x);
    }
}
...
{
    print("\n##PERF_START\n");
    mini_printf("##META:suite=atomik_v3,cpu_khz=21600,atomik=direct_wire,banks=1,width=64\n");
