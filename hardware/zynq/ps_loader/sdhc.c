...
sdhc_err_t sdhc_card_reselect(sd_card_t *c, uint32_t rca)
{
    ...
    c->version2 = 1;
    c->sdhc     = 1;
    /* Card is in Transfer state from ps_loader — skip CMD7.
     * Only do SW_RESET_CMD if CMD_INHIBIT_CMD is stuck from prior failures.
     * SW_RESET_CMD kills CLOCK_CTRL (INTCLK_EN→0); restore clock if that happens.
     * On a fresh board, CMD_INHIBIT_CMD=0 so we skip the reset entirely. */
    if (r32(c->base + SDHC_PRESENT_STATE) & PRES_CMD_INHIBIT_CMD) {
        w8(c->base + SDHC_SW_RESET, 0x02u);
