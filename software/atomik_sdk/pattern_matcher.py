...
class Pattern:
    ...
    def matches(self, delta: int) -> MatchResult | None:
...
class PatternMatcher:
    ...
    def __init__(
        self,
        vocab: list[str | int | Pattern] | None = None,
        ...
        self.default_pattern = Pattern(
            id=-1, name=default_pattern_name, signature=0, min_confidence=0.0
        )
    ...
    def match(self, delta: int) -> MatchResult | None:
        """
    ...
    def load_vocab(self, path: str | Path) -> None:
        ...
        for p in data.get("patterns", []):
            mask_val = p.get("mask", "0xFFFFFFFFFFFFFFFF")
            if isinstance(mask_val, str):
                mask_val = int(mask_val, 16)
            self.add_pattern(
                pattern=p["signature"],
                pattern_id=p.get("id"),
                name=p.get("name"),
