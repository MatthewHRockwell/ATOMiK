...
into ATOMiK's 4×4×4 voxel representation with configurable binarization.
"""
...
class Voxel:
    """
    A 4×4×4 spatiotemporal voxel encoded as a 64-bit word.
    ...
    @classmethod
    def from_bytes(cls, data: bytes, position: Tuple[int, int, int] = (0, 0, 0)) -> Voxel:
...
class VoxelEncoder:
    ...
    The VoxelEncoder implements ATOMiK's 4×4×4 spatiotemporal encoding:
    - 4 temporal frames
    - 4×4 spatial tile
    ...
    def __init__(
        self,
        tile_method: Union[str, TileMethod] = TileMethod.OTSU,
    ...
    def encode_frames(self, frames: np.ndarray) -> List[Voxel]:
        ...
        for ti in range(0, T - t + 1, t):
            for yi in range(0, H - h + 1, h):
                for xi in range(0, W - w + 1, w):
