...
## Tightened In 2026-05-23 Pitch-Readiness Pass

- Updated pitch deck source, one-pager, investor FAQ, and data-room planning
  documents around the customer-value narrative: less heat, longer battery life,
  lower bandwidth pressure, faster local state paths, and smaller hardware
  profiles.
- Promoted `website/public/09-current-live-atomik-desk-v039k.png` as the current
  `HARDWARE_VALIDATED` proof image for investor materials.
- Added `BUILD_ARTIFACT` as an evidence tier for locally assembled boot artifacts
  that are not yet live end-to-end hardware results.
- Replaced the stale deck generator and disabled legacy deck updater scripts so
  old metric-heavy slides are not regenerated by accident.
- Kept power, thermal, water, battery, and footprint improvements framed as
  evaluation targets until workload-specific measured artifacts exist.
